...
describe('LoginController', () => {
  ...
  describe('throws', () => {
    ...
    it('should return correct http response if userService throws', async () => {
      const { sut, userServiceSpy } = makeSut()
      jest.spyOn(userServiceSpy, 'getByEmail').mockImplementationOnce(throwError)
      const httpRequest = makeFakeRequest()
      const result = await sut.handle(httpRequest)
      expect(result).toEqual({
        statusCode: 500,
    ...
    it('should return correct http response if hasher throws', async () => {
      const { sut, hasherSpy } = makeSut()
      jest.spyOn(hasherSpy, 'compare').mockImplementationOnce(throwError)
      const httpRequest = makeFakeRequest()
      const result = await sut.handle(httpRequest)
      expect(result).toEqual({
        statusCode: 500,
